...
class JatosStudyRunSimulation extends Simulation {
...
//    .wsBaseUrl("wss://cortex.jatos.org")
  ...
  val scn = scenario("JatosStudyRunSimulation")
...
// ### 2. Component ###
  .exec(
    ...
    http("Post study session data").post("/publix/${studyId}/studySessionData?srid=${studyResultId}").headers(header_ajax).body(StringBody("""{"bar":"foo"}"""))
  ).exec(
