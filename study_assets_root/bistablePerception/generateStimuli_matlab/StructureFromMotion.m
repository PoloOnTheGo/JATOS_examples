...
%[window,windowRect]=Screen('OpenWindow',screenid, black, [0 0 712 512], 32, 2 );
...
centeredRect = CenterRect([0 0 900 700], Screen('Rect', screenid));

movie = Screen('CreateMovie', window, 'MyTestMovie.mov', centeredRect(3), centeredRect(4), 30, ':CodecSettings=Videoquality=0.5 Profile=2');
...
while ~KbCheck
    ...
    Screen('DrawDots', window, [xpos; ypos], dotSizePixels, white, center, 2);

    % Flip to the screen
    Screen('AddFrameToMovie', window, centeredRect);
    Screen('Flip', window);
